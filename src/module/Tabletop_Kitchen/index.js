...
export const TableTopKitchen = ({
  ...
  console.log("TableTopKitchen", getAllProduct);
  let filterTableKitchenFunc = getAllProduct.filter(
    (value) => value.category_id == 2
  );
  window.addEventListener("load",() =>{
    getProductListCategory([]);
 });
  console.log(filterTableKitchenFunc);
  return (
    <>
      <Navbar />
      <div className="Tabletop_section">
        <div className="Tabletop_banner_section">
          <h1>Tabletop & Kitchen</h1>
        ...
        <div className="Tabletop_showcase">
          <div className="cards_product">
          {ProudctListCategory.length == 0 ? (
             <>
              {filterTableKitchenFunc.map((value) => {
